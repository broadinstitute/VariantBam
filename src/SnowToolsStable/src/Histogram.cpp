...
#ifdef BINARY_SEARCH
  std::vector<size_t>::const_iterator it = std::upper_bound(m_ind.begin(), m_ind.end(), elem);
...
#else
  for (auto& bin : m_bins)
    if (bin.contains(elem)) {
      --bin;
      return;
    }
#endif

  std::cerr << "element not removed from any bin " << elem << std::endl;
  //exit(EXIT_FAILURE);
}
...
void Histogram::addElem(const int32_t& elem) {
  ...
  std::cerr << "element not inserted into any bin " << elem << std::endl;
  //exit(EXIT_FAILURE);
}
